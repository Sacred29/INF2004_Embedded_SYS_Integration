...
void log_ir_data(const char *ir_data)
{
    printf("IR Data Received: %s\n", ir_data);
}
...
void handle_client(int client_socket)
{
    ...
    while ((bytes_received = recv(client_socket, buffer, BUFFER_SIZE - 1, 0)) > 0)
    {
        ...
        {
            // Handle IR data
            log_ir_data(buffer + 2); // Process IR data (remove "I:" prefix)
        }
        else
        {
            printf("Unknown data type received: %s\n", buffer);
        }
    }

...
void wifi_send_receive_task(__unused void *params)
{
    ...
    // Step 1: Set up the remote control server
    ...
    // Step 2: Connect to the telemetry server
    ...
    // Step 3: Handle both roles (remote control and telemetry)
    fd_set read_fds;
    char buffer[WIFI_MESSAGE_BUFFER_SIZE];
    char telemetry_message[WIFI_MESSAGE_BUFFER_SIZE];
    ...
    while (1)
    {
        // Remote Control Server: Accept connections and handle motor commands
        FD_ZERO(&read_fds);
        FD_SET(receive_sock, &read_fds);

        struct timeval timeout = {.tv_sec = 1, .tv_usec = 0}; // 1-second timeout
        int activity = select(receive_sock + 1, &read_fds, NULL, NULL, &timeout);
        if (activity < 0)
        {
            printf("Select error.\n");
            break;
        ...
        // Telemetry Client: Send telemetry data
...
void vLaunch(void)
{
    ...
    if (xTaskCreate(wifi_send_receive_task, "WiFiSendReceiveTask", WIFI_RECEIVE_TASK_STACK_SIZE, NULL, WIFI_RECEIVE_TASK_PRIORITY, NULL) != pdPASS)
    {
